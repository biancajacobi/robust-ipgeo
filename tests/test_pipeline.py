"""Tests für die Eval-Pipeline: Auflösen je IP, Schätzen, Bewerten."""
...
# Anchor in Berlin; drei Quellen: zwei nah dran, eine grob daneben (London).
...
OBSERVATIONS = [
    ...
    {"ip": "1.2.3.4", "source": "ipapi_co", "lat": "51.51", "lon": "-0.13",
     "status": "success", "fetched_at_utc": "2026-06-03T00:00:00Z"},
    # fehlgeschlagene Quelle wird ignoriert
...
def test_load_cases_filters_and_builds_points():
    ...
    assert case["points"].shape == (3, 2)          # http_error-Quelle raus
    assert set(case["sources"]) == {"ip_api", "ipwho_is", "ipapi_co"}
    assert len(case["provenance"]) == 3            # FF4: Rückverfolgbarkeit
...
def test_evaluate_robust_beats_centroid():
    ...
    # je Schätzer eine aggregate-Zeile + drei source-Zeilen
    agg = df[df["kind"] == "aggregate"].set_index("estimator")["error_km"]
    assert set(agg.index) == set(BASELINES)
    assert (df["kind"] == "source").sum() == 3
    # der London-Ausreißer zieht den Centroid deutlich stärker als den geom. Median
...
def test_line_weights_split_per_lineage():
    import numpy as np
    # zwei Quellen teilen sich eine Linie, eine eigene Linie
...
def test_evaluate_line_weighted_column(monkeypatch):
    ...
    # lineage an die Observations hängen: ip_api eigene Linie, die zwei anderen geteilt
